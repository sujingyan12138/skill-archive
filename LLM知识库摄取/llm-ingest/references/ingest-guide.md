...
## 核心原则：摄取只读取已经归档好的 raw

**这是本 skill 最重要的原则。**

raw/ 目录是知识库的「源代码」层，必须在摄取开始前满足：
...
3. **中文**：非中文来源已在归档阶段保存完整中文译文，并尽量保留原文或原始附件。

4. **不可修改**：`llm-ingest` 永不编辑 raw/ 目录。如果来源不完整，先返回 `archive-to-raw` 补齐。
...
### 步骤 1：确认并读取 raw 来源
- 确认来源已经位于 `raw/` 对应分类目录
- 检查正文、来源 URL、作者、抓取时间和必要附件是否完整
- 把 raw 当作只读输入；如果不完整，先停止并使用 `archive-to-raw` 补齐
- 用户只要求“入库到 raw”时不执行本流程
...
- 原始来源：raw/articles/xxx.md（摄取阶段未修改）
